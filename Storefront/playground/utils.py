...
class AutomationMethods:
    ...
    def set_interfaceconfigration(selected_host,interface_name,ipv4):
        ...
        print(status)
        return status
    def set_switchgateway(selected_host,interface_name,ipv4):
        print("sucess")
        print(selected_host)
        print(interface_name)
        print(ipv4)
        status = 'ok'#configration.Switch_gateway(selected_host,interface_name) #"ok"
        print(status)
        return status
    ...
    def Vlans_configs(selected_hosts,interfaces_list,vlan_cidr, 
        ...
        #status = configration.set_valnconfigration(selected_hosts[0],interfaces_list,vlan_cidr, 
        #                vlan_id, vlan_name,tag)
        status = "ok"
        print(status)
        return status
    ...
    def display_VLAN_Brief():
       return "hello VALN Brief"#show.vlan_information()

    def display_OSPF_Neighbor_Information():
       return "hello OSPF"#show.ospf_neighbors

    def display_OSPF_Database_Summary():
       return "hello OSPF db"#show.ospf_database
